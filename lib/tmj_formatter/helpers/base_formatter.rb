...
module TMJFormatter
  class BaseFormatter < RSpec::Core::Formatters::BaseFormatter
    DEFAULT_OPTIONS = { update_existing_tests: false, test_owner: nil, custom_labels: [] }.freeze
    ...
    def process_example(example)
      {
          ...
          "labels":       configure_labels(example),
    ...
    def configure_labels(example)
      return @options[:custom_labels] if example.metadata[:label].nil?
      @options[:custom_labels].push(example.metadata[:label]).flatten!
    end
